...
export function createGame({size=12,budget=45,mode='standard',repeatOnHit=false,difficulty='normal',roster,mines=0}={}){
 ...
 while(remaining>=4&&botRoster.length<12){const reserve=Math.max(0,minimumShips(size)-botRoster.length-1)*4;const choices=preferred.filter(t=>def(t).cost<=remaining-reserve);if(!choices.length)break;const type=pick(choices);botRoster.push({type});remaining-=def(type).cost;}
 ...
 autoDeploy(state,0);autoDeploy(state,1);return state;
}
export function autoDeploy(state,player=0){
 ...
 if(!ok)continue;for(const m of p.mines){const available=[];for(let y=0;y<state.size;y++)for(let x=0;x<state.size;x++)if(!p.ships.some(s=>s.cells.some(c=>c.x===x&&c.y===y))&&!p.mines.some(n=>n.x===x&&n.y===y))available.push({x,y});if(!available.length){ok=false;break;}Object.assign(m,pick(available));}if(ok)return state;
...
function hit(state,who,x,y,depth=false){const p=state.players[who],enemy=state.players[1-who];const mine=enemy.mines.findIndex(m=>m.x===x&&m.y===y);if(mine>=0&&!depth){enemy.mines.splice(mine,1);p.shots[key(x,y)]={kind:'hit'};say(state,`${who===0?'You':'Bot'} hit ${coord(x,y)}: section still intact.`);return{hit:true};}
 ...
 const c=s.cells.find(c=>c.x===x&&c.y===y&&c.hp>0);c.hp--;p.shots[key(x,y)]={kind:c.hp?'hit':'destroyed'};say(state,`${who===0?'You':'Bot'} hit ${coord(x,y)}: ${c.hp?'section still intact':'section destroyed'}.`);
...
function resolve(state,a){
 ...
 requireTarget(state,a.x,a.y);const c=s.cells.find(c=>c.x===a.x&&c.y===a.y);if(!s.repairs)throw Error('No repairs remaining.');if(!c||c.hp>=c.max)throw Error('Select a damaged section of this ship.');c.hp++;s.repairs--;say(state,`${who===0?'Your':'Enemy'} fleet repaired a section.`);
 }else{
 if(def(s.type).ability!==a.kind||s.uses<=0)throw Error('This ability is unavailable.');
 ...
 if(a.kind==='scan')requireTarget(state,a.x,a.y,3,3);else requireTarget(state,0,a.y);
 const count=enemy.ships.filter(e=>a.kind==='scan'?!def(e.type).submarine:!!def(e.type).submarine).flatMap(e=>e.cells).filter(c=>c.hp>0&&(a.kind==='scan'?c.x>=a.x&&c.x<a.x+3&&c.y>=a.y&&c.y<a.y+3:c.y===a.y)).length;
 // Private scan results are stored per player and filtered out of opponent views.
 p.scans??=[];p.scans.push({kind:a.kind,x:a.x??0,y:a.y,count});say(state,who===0?`${a.kind==='scan'?'Aircraft scan':'Hydrophone'} at ${coord(a.x??0,a.y)}: ${count} intact ${a.kind==='scan'?'surface':'submarine'} sections.`:`Enemy used ${a.kind==='scan'?'aircraft reconnaissance':'hydrophone'}.`);
 }else{const w=a.kind==='depth'?3:2,h=a.kind==='depth'?3:1;requireTarget(state,a.x,a.y,w,h);for(let dy=0;dy<h;dy++)for(let dx=0;dx<w;dx++)hit(state,who,a.x+dx,a.y+dy,a.kind==='depth');}
 s.uses--;
 }
