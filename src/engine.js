...
function resolve(state,a){
 ...
 if(def(s.type).ability!==a.kind||s.uses<=0||(a.kind==='scan'&&!(s.planes?.length)))throw Error(a.kind==='scan'?'Place a reconnaissance plane on the carrier first.':'This ability is unavailable.');
 ...
 p.scans??=[];p.scans.push({kind:a.kind,x:a.x??0,y:a.y,count});if(a.kind==='scan'&&s.planes?.length)s.planes.shift();say(state,who===0?`${a.kind==='scan'?'Aircraft scan':'Hydrophone'} at ${coord(a.x??0,a.y)}: ${count} intact ${a.kind==='scan'?'surface':'submarine'} sections.`:`Enemy used ${a.kind==='scan'?'aircraft reconnaissance':'hydrophone'}.`);
